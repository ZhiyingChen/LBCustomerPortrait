...
class NoHeaderTable:
    def __init__(self, parent, columns, col_widths=None, height=5, col_stretch=None, show_header=False):
        self.root = parent
        self.frame = tk.Frame(parent)
        self.tooltip = None  # 初始化 tooltip
        self.tree = ttk.Treeview(
        ...
        # 设置列宽和列名
        for i, col in enumerate(columns):
            width = col_widths[i] if col_widths and i < len(col_widths) else 50
            stretch = col_stretch[i] if col_stretch and i < len(col_stretch) else True
            ...
            if show_header:
                ...
                if i == 0:
                    self.tree.column("#0", width=width, stretch=stretch)
                else:
                    self.tree.column(col, width=width, stretch=stretch)
